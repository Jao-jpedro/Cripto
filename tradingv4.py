...
ROI_HARD_STOP = -0.05  # trava de segurança: ROI nunca abaixo de -5%


def close_if_abs_loss_exceeds_5c(dex, symbol, current_px: float, *, vault) -> bool:
        try:
            qty, _, entry_px, side = _get_pos_size_and_leverage(dex, symbol, vault=vault)
        except Exception:
...
def close_if_breached_leveraged(dex, symbol, current_px: float, *, vault, buffer_pct: float = LIQUIDATION_BUFFER_PCT) -> bool:
        try:
        ...
        try:
            qty, _, _, side_confirm = _get_pos_size_and_leverage(dex, symbol, vault=vault)
        except Exception:
...
def _compute_roi_from_price(entry: float, side: str, price: float, leverage: float = 1.0):
        try:
            entry_f = float(entry)
            price_f = float(price)
            lev_f = max(float(leverage or 1.0), 1.0)
        except Exception:
            return None
...
def close_if_roi_breaches(dex, symbol, current_px: float, *, vault, threshold: float = ROI_HARD_STOP) -> bool:
        ...
        if roi_f is None:
            ...
            s = str(side).lower()
            if s in ("long", "buy"):
                roi_f = ((px_f - entry_f) / entry_f) * float(lev or 1.0)
            elif s in ("short", "sell"):
                roi_f = ((entry_f - px_f) / entry_f) * float(lev or 1.0)
            else:
                return False
        else:
            # ROI fornecido geralmente é percentual (alavancado). Caso esteja em decimal mas sem alavancagem,
            # multiplicamos se magnitude sugere valor pequeno.
            try:
                if abs(roi_f) < 1.0 and float(lev or 1.0) > 1.0:
                    roi_f *= float(lev or 1.0)
            except Exception:
                pass
        ...
        if roi_f is not None and roi_f <= effective_threshold:
            try:
                if not side or qty <= 0:
                    return False
                exit_side = "sell" if (str(side).lower() in ("long", "buy")) else "buy"
...
def guard_close_all(dex, symbol, current_px: float, *, vault) -> bool:
        try:
            if close_if_roi_breaches(dex, symbol, current_px, vault=vault, threshold=ROI_HARD_STOP):
                return True
        except Exception:
            pass
        try:
    ...
    class EMAGradientStrategy:
        ...
        def _ensure_position_protections(self, pos: Dict[str, Any], df_for_log: Optional[pd.DataFrame] = None):
            ...
            try:
                ...
                if guard_close_all(self.dex, self.symbol, float(current_px), vault=HL_SUBACCOUNT_VAULT):
