...
# ⚠️  VERIFICAÇÃO DE SEGURANÇA DA SUBCONTA
def _verify_subaccount_config():
    """Verifica se a subconta está corretamente configurada"""
    target_subaccount = "0x5ff0f14d577166f9ede3d9568a423166be61ea9d"
    
    # Verificar se temos apenas uma configuração de carteira
    if len(WALLET_CONFIGS) != 1:
        _log_global("SECURITY", f"❌ ERRO: Deve haver exatamente 1 carteira configurada, encontradas: {len(WALLET_CONFIGS)}", "ERROR")
        return False
    
    wallet_config = WALLET_CONFIGS[0]
    
    # Verificar se a subconta está configurada corretamente
    if wallet_config.vault_address != target_subaccount:
        _log_global("SECURITY", f"❌ ERRO: Subconta incorreta! Esperada: {target_subaccount}, Configurada: {wallet_config.vault_address}", "ERROR")
        return False
    
    _log_global("SECURITY", f"✅ Subconta configurada corretamente: {target_subaccount}", "INFO")
    _log_global("SECURITY", f"✅ Todas as operações serão executadas na subconta especificada", "INFO")
    return True
...
class TradingLearner:
    ...
    def extract_features_raw(self, symbol: str, side: str, df: pd.DataFrame, price: float) -> dict:
        ...
        try:
            # Verificar se tem colunas básicas necessárias
            required_cols = ['valor_fechamento']
            missing_cols = [col for col in required_cols if col not in df.columns]
            if missing_cols:
                _log_global("LEARNER", f"Missing required columns: {missing_cols}", "WARN")
                return {}
...
# ⚠️  IMPORTANTE: TODAS AS OPERAÇÕES SERÃO EXECUTADAS NA SUBCONTA CONFIGURADA ABAIXO
# Subconta especificada: 0x5ff0f14d577166f9ede3d9568a423166be61ea9d
WALLET_CONFIGS = [
    WalletConfig(
        name="CARTEIRA_PRINCIPAL",
        wallet_address="0x08183aa09eF03Cf8475D909F507606F5044cBdAB",  # Conta principal para autenticação
        private_key_env="HYPERLIQUID_PRIVATE_KEY",
        vault_address="0x5ff0f14d577166f9ede3d9568a423166be61ea9d",  # ⚠️  SUBCONTA ESPECÍFICA - TODAS AS OPERAÇÕES AQUI
        usd_per_trade=3.0    # $3 por operação na subconta
    )
]
...
def check_all_wallet_balances():
    ...
    for wallet_config in WALLET_CONFIGS:
        try:
            ...
            # Para subconta configurada, verificar saldo da vault address
            if wallet_config.vault_address:
                ...
                if vault_value > 0:
                    ...
                    usdc_used = 0.0
                    wallet_info = f"💰 {wallet_config.name} (Subconta): ${usdc_total:.2f} USDC | Trade: ${wallet_config.usd_per_trade}/op"
                else:
...
# ===============================================================================
# NOVA ESTRATÉGIA: RATIO AVG_BUY/SELL SIMPLES
# ===============================================================================
# LÓGICA DE ENTRADA/SAÍDA:
# - Entrada LONG: quando avg_buy/sell passa de <1.0 para >1.0 
# - Entrada SHORT: quando avg_buy/sell passa de >1.0 para <1.0
# - Saída: inversão do ratio (se long e ratio <1.0 = sair, se short e ratio >1.0 = sair)
# - Stop Loss: 20% fixo para proteção de risco
# - Assets: apenas PUMP e AVNT
# - Valor por trade: $3 na subconta configurada
...
class SimpleRatioStrategy:
                ...
                for wallet_config in WALLET_CONFIGS:
                    try:
                        ...
                        if wallet_config.vault_address:
                            ...
                            if vault_value > 0:
                                usdc_total = vault_value
                                wallet_info = f"💰 {wallet_config.name} (Subconta): ${usdc_total:.2f} USDC"
                            else:
                                balance = dex_instance.fetch_balance()
                                usdc_total = balance.get("USDC", {}).get("total", 0.0)
    ...
    def _abrir_posicao_dual_wallet(self, side: str, usd_to_spend: float, df_for_log: pd.DataFrame, atr_last: Optional[float] = None):
        ...
        # Registrar tempo de entrada da posição para controle de 4 horas
        self._position_entry_time = _time.time()
        self._log(f"[DEBUG_POSITION_TIME] Posição aberta às {_time.strftime('%H:%M:%S %d/%m/%Y', _time.localtime(self._position_entry_time))}", level="DEBUG")
    ...
    def _fechar_posicao(self, df_for_log: pd.DataFrame):
        ...
        try:
            ...
            # Limpar timestamp de entrada da posição
            self._position_entry_time = None
            self._log("[DEBUG_POSITION_TIME] Timestamp de entrada limpo após fechamento", level="DEBUG")
    ...
    def step(self, df: pd.DataFrame, usd_to_spend: float = None, rsi_df_hourly: Optional[pd.DataFrame] = None):
        """
        NOVA ESTRATÉGIA SIMPLES: RATIO AVG_BUY/SELL
        
        Lógica:
        1. Calcular ratio atual avg_buy/sell 
        2. Detectar inversões (>1.0 para <1.0 ou vice-versa)
        3. Entrar LONG quando ratio passa de <1.0 para >1.0
        4. Entrar SHORT quando ratio passa de >1.0 para <1.0  
        5. Sair quando ratio inverte novamente
        6. Stop Loss fixo 20%
        """
        ...
        usd_to_spend = usd_to_spend or self.cfg.USD_PER_TRADE
        
        try:
            # 1. CALCULAR RATIO AVG_BUY/SELL ATUAL
            current_ratio = self._calculate_avg_buy_sell_ratio(df)
            if current_ratio is None:
                self._log("Não foi possível calcular ratio avg_buy/sell", level="WARN")
                return
                
            # 2. VERIFICAR POSIÇÃO ATUAL
            pos = self._posicao_aberta()
            current_pos_side = self._norm_side(pos.get("side")) if pos else None
            
            # 3. DETECTAR MUDANÇA DE ESTADO DO RATIO
            ratio_crossed_up = self._detect_ratio_cross(current_ratio, direction="up")     # <1.0 → >1.0
            ratio_crossed_down = self._detect_ratio_cross(current_ratio, direction="down") # >1.0 → <1.0
            
            # 4. LÓGICA DE ENTRADA/SAÍDA
            if pos:
                # TEM POSIÇÃO: verificar saída por inversão do ratio ou stop loss
                self._check_exit_conditions(pos, current_pos_side, current_ratio, df)
            else:
                # SEM POSIÇÃO: verificar entrada por inversão do ratio
                if ratio_crossed_up:
                    self._log(f"🟢 RATIO CROSS UP: {current_ratio:.3f} - ENTRADA LONG", level="INFO")
                    self._enter_position("buy", usd_to_spend, df)
                elif ratio_crossed_down:
                    self._log(f"🔴 RATIO CROSS DOWN: {current_ratio:.3f} - ENTRADA SHORT", level="INFO")
                    self._enter_position("sell", usd_to_spend, df)
                    
            # 5. ATUALIZAR HISTÓRICO DO RATIO
            self._update_ratio_history(current_ratio)
            
            if self.debug:
                self._log(f"📊 RATIO DEBUG: atual={current_ratio:.3f} | pos={current_pos_side or 'None'} | histórico={[f'{r:.3f}' for r in self._ratio_history[-3:]]}", level="DEBUG")
    ...
    def _calculate_stop_price(self, entry_price: float, side: str) -> Optional[float]:
        ...
        if not self._first_step_done:
            ...
            if pos_now and float(pos_now.get("contracts", 0)) > 0:
                ...
                # Inicializar timestamp para posição preexistente (estimativa conservadora)
                if self._position_entry_time is None:
                    self._position_entry_time = _time.time()
                    self._log(f"[DEBUG_POSITION_TIME] Timestamp inicializado para posição preexistente às {_time.strftime('%H:%M:%S %d/%m/%Y', _time.localtime(self._position_entry_time))}", level="DEBUG")
        ...
        if prev_side and not pos:
            ...
            # Limpar timestamp de entrada quando posição é fechada externamente
            self._position_entry_time = None
            self._log("[DEBUG_POSITION_TIME] Timestamp de entrada limpo após fechamento externo", level="DEBUG")
        ...
        if entry_px > 0 and qty_pos > 0 and px_now > 0:
            ...
            # Verificação de saída por tempo: 4 horas
            if self._position_entry_time is not None:
                current_time = _time.time()
                time_in_position = current_time - self._position_entry_time
                time_limit_4h = 4 * 60 * 60  # 4 horas em segundos
                
                time_remaining = time_limit_4h - time_in_position
                hours_remaining = time_remaining / 3600
                
                if self.debug:
                    self._log(f"[DEBUG_TIME_EXIT] Tempo na posição: {time_in_position/3600:.2f}h | Restante: {hours_remaining:.2f}h", level="DEBUG")
                
                if time_in_position >= time_limit_4h:
                    self._log(
                        f"SAÍDA POR TEMPO: Posição aberta há {time_in_position/3600:.2f} horas (limite 4h). Fechando posição automaticamente.",
                        level="WARN",
                    )
                    self._fechar_posicao(df_for_log=df)
                    return
