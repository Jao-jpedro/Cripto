...
class TradeLogger:
    def __init__(self, df_columns: pd.Index,
                 csv_path="/dbfs/FileStore/trade_log.csv",
                 xlsx_path_dbfs="/dbfs/FileStore/trade_log.xlsx"):
        ...
        if _has_dbutils():
            try:
                ...
                try:
                    dbutils.fs.head("dbfs:/FileStore/trade_log.xlsx", 1)
                except Exception:
                    need_bootstrap = True
                if need_bootstrap:
                    pd.DataFrame(columns=self.all_cols).to_excel(self.xlsx_tmp, index=False)
                    dbutils.fs.cp(f"file:{self.xlsx_tmp}", f"dbfs:{self.xlsx_path_dbfs}", True)
            except Exception:
                pass
    ...
    def _save_xlsx_dbfs(self, df_all: pd.DataFrame):
        df_all.to_excel(self.xlsx_tmp, index=False)
        if _has_dbutils():
            dbutils.fs.cp(f"file:{self.xlsx_tmp}", f"dbfs:{self.xlsx_path_dbfs}", True)
        else:
            with open(self.xlsx_path_dbfs, "wb") as f_out, open(self.xlsx_tmp, "rb") as f_in:
    ...
    def append_event(self, df_snapshot: pd.DataFrame,
        ...
        try:
            ...
            links = (
                "<p>✅ Histórico atualizado. Baixe os arquivos:</p>"
                "<ul>"
                "<li><a href='/files/trade_log.csv' download>📥 trade_log.csv</a></li>"
                "<li><a href='/files/trade_log.xlsx' download>📥 trade_log.xlsx</a></li>"
                "</ul>"
            )
        except Exception as e:
            links = (
                f"<p>⚠️ XLSX não pôde ser atualizado ({type(e).__name__}: {e}). "
                "O CSV foi salvo normalmente.</p>"
                "<ul><li><a href='/files/trade_log.csv' download>📥 trade_log.csv</a></li></ul>"
            )
        try:
            displayHTML(links)
        except Exception:
...
class EMAGradientStrategy:
    ...
    def _safe_log(self, evento: str, df_for_log: Optional[pd.DataFrame], **kwargs):
        ...
        if isinstance(df_for_log, pd.DataFrame) and len(df_for_log) > 0:
            try:
                cols_pref = [c for c in ["data", "valor_fechamento", "ema_short", "ema_long"] if c in df_for_log.columns]
                snap = df_for_log[cols_pref].tail(1) if cols_pref else df_for_log.tail(1)
            except Exception:
    ...
    def print_local_log_tail(self, n: int = 10):
        """Imprime os últimos n eventos do buffer local."""
        rows = self.local_log_tail(n)
        if not rows:
            print("ℹ️ Buffer local vazio.")
            return
...
# Salva como um único arquivo CSV no FileStore
df_pandas.to_csv("/dbfs/FileStore/previsoes_df.csv", index=False)

# Gera o link de download
displayHTML("<a href='/files/previsoes_df.csv' download>📥 Baixar CSV</a>")
