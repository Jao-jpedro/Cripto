...
# === Trailing ROI Manager (10 p.p., monotônico) ===
_TRAIL_STATE = {
...
class TrailingROIManager:
    ...
    def _k(self, symbol: str, side: str) -> str:
        s = (side or "").lower()
        if s in ("buy","long"): s = "buy"
        elif s in ("sell","short"): s = "sell"
        return f"{symbol}:{s}"
    ...
    def update(self, *, symbol: str, side: str, entry_px: float, current_px: float, lev: float, qty: float, trail_pp: float = 0.10):
        if entry_px <= 0 or current_px <= 0 or lev <= 0 or qty <= 0:
            return None
        side = (side or "").lower()
        side = "buy" if side in ("buy","long") else ("sell" if side in ("sell","short") else side)
...
def ensure_tpsl_for_position(dex, symbol, *, vault, retries: int = 2, price_tol_pct: float = 0.001):
    """
    Gestão de saída via Trailing ROI (10 p.p.) + hard stop -$0,10. Sem TP/SL fixos.
    ...
    try:
        ...
        current_px = float(t.get("last") or t.get("close") or (t.get("info") or {}).get("markPx") or entry)
    except Exception:
        current_px = float(entry)
