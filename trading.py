...
class TradingFuturo100:
    ...
    def run_live_monitoring(self):
        ...
        print("\n" + "="*80)
        print("🔴 MODO MONITORAMENTO CONTÍNUO INFINITO - TODOS OS ATIVOS")
        print("="*80)
        ...
        print(f"📝 Log de trades: {self.trades_file}")
        print(f"🎯 {len(self.symbols)} ativos monitorados")
        print("♾️ Execução INFINITA - rode para sempre!")
        ...
        try:
            while True:
                ...
                if elapsed_time - last_heartbeat >= 600:  # 10 minutos
                    hours = elapsed_time / 3600
                    print(f"\n💓 HEARTBEAT - Sistema rodando há {hours:.1f} horas | {cycle_count} ciclos completados")
                    last_heartbeat = elapsed_time
                
                trades_this_cycle = 0
                print(f"\n{'='*80}")
                print(f"🔄 CICLO #{cycle_count} | {datetime.now().strftime('%H:%M:%S')} | ⏱️ {elapsed_time/60:.1f}min ({elapsed_time/3600:.2f}h)")
                print(f"{'='*80}")
                ...
                print(f"⏰ Tempo total rodando: {elapsed_time/3600:.2f} horas")
                
                if trades_this_cycle > 0:
                    print(f"🎉 {trades_this_cycle} NOVOS TRADES EXECUTADOS!")
                
                # Aguardar 1 minuto entre ciclos
                sleep_time = 60
                print(f"💤 Aguardando {sleep_time/60:.0f} minuto até próximo ciclo...")
                
                for i in range(int(sleep_time)):
                    if i % 30 == 0 and i > 0:
                        print(f"  ⏳ {(sleep_time-i)} segundos restantes...")
                    time.sleep(1)
                
        except KeyboardInterrupt:
            print(f"\n⏹️ Sistema interrompido pelo usuário após {cycle_count} ciclos")
            print(f"⏰ Tempo total de execução: {elapsed_time/3600:.2f} horas")
        
        print(f"📊 Execução finalizada: {cycle_count} ciclos, {elapsed_time/3600:.2f} horas")
        self.print_results()
